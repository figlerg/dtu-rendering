...
bool Plane::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
  ...
  //       the dot product of the vectors v and w.
	...
	if (abs(dot(r.direction, onb.m_normal)) <= 1e-07) {
		return false;
	};
	...
	bool in_interval = (t_prime <= r.tmax) && (t_prime >= r.tmin);

	// set everything accordingly
	hit.has_hit = in_interval;
	hit.dist = t_prime; // hope it's normalized?
...
{ 
  ...
  // tex_scale          (constant for scaling the texture coordinates)
	float3 b1 = onb.m_tangent;
	float3 b2 = onb.m_binormal;

	u = tex_scale * dot(b1, hit_pos - position);
	v = tex_scale * dot(b2, hit_pos - position);
  //u = 0.0f;
  //v = 0.0f;
}